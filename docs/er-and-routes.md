# ER 図・テーブル案・URL 設計（MINNANOKOE / Survivor+）
...
| -------- | ----------------------------------- | ------------------------------------------------------- |
| GET      | `/`                                 | トップ                                                  |
| GET      | `/stories`                          | みんなの声を読む（検索・タグ・一覧のハブ）              |
...
- UI 文言「みんなの声を読む」「みんなの声を読む」の表記ゆれは、ルートは `/stories` に一本化しラベルのみ揃えるとよい。
- アプリ名は表示で **MINNANOKOE** に統一（「みんなの声時点」は誤記の可能性）。
